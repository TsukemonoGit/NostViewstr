<script lang="ts">
	...
	import { contentStore, ogpStore, URLPreview } from '$lib/stores/bookmarkEvents';
...
{#if tag.some((tag) => tag[0] === 'content-warning') && view == false}
	<button type="button" class="btn variant-filled-warning m-0" on:click={clickView}>
		content-warning<br />
							...
							{#if new URL(item.content).hostname.endsWith('twitter.com')}
								...
								>
									<blockquote class="twitter-tweet">
										<p lang="ja" dir="ltr">
											<a class="anchor example break-all whitespace-pre-wrap" href={item.content}
												>{item.content}</a
											>
										</p>
									</blockquote>
